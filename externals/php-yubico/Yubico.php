...
class Auth_Yubico
{
	...
	function getParameters($parameters)
	{
	  ...
	  foreach ($parameters as $param) {
	    if(!preg_match("/" . $param . "=([0-9]+)/", $this->_response, $out)) {
          return (new PEAR)->raiseError('Could not parse parameter ' . $param . ' from response');
	    }
			...
			$sl=null, $timeout=null)
	{
	  ...
	  if (!$ret) {
	    return (new PEAR)->raiseError('Could not parse Yubikey OTP');
	  }
	  ...
	  if ($replay) return (new PEAR)->raiseError('REPLAYED_OTP');
	  if ($valid) return true;
	  return (new PEAR)->raiseError('NO_VALID_ANSWER');
	}
}
?>
